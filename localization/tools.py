...
def calc_hydrophones_distances(hydrophones_coords):
    ...
    """
    hydrophones_dist_matrix = np.empty(
        (len(hydrophones_coords), len(hydrophones_coords))
    )
...
def find_audio_files(filename, hydrophones_config):
    ...
    for row_idx, row_data in hydrophones_config.iterrows():
        file_path = os.path.join(
            row_data.data_path, row_data.file_name_root + file_tail
        )
...
def stack_waveforms(audio_files, detec, TDOA_max_sec):
    waveform_stack = []
    for audio_file, channel in zip(
        audio_files["path"], audio_files["channel"]
    ):  # for each channel
        # load waveform
...
def plot_all_channels(
    ...
    graph_spectros = GrapherFactory(
        "SoundPlotter", title="Spectrograms", frequency_max=fmax
    )
    ...
    if detections:
        graph_spectros.add_annotation(
            detections,
            panel=detections_channel,
            color="green",
            label="Detections",
        )
        graph_waveforms.add_annotation(
            detections,
            panel=detections_channel,
            color="green",
            label="Detections",
...
def plot_single_channel(
    ...
    if detections:
        graph.add_annotation(
            detections, panel=0, color="green", label="Detections"
        )  # overlay detections on waveform plot
        graph.add_annotation(
            detections, panel=1, color="green", label="Detections"
        )  # overlay detections on spectrogram plot
